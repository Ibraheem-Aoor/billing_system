  ...
  <div>
    <div class="content-wrapper">
      <div class="d-flex align-items-center justify-content-between">
        ...
        <div class="mb-4">
          <button
            v-if="$can('update_invoices')"
            class="btn btn-warning btn-with-shadow mr-1"
            type="button"
            @click="editInvoice"
          >
          ...
          <div id="print-invoice" class="cus-invoice_container">
            <div class="cus-invoice_container__item cus-px-5 cus-mt-5">
              <div class="cus-w-25 cus-f-left cus-p-2">
                <div>
                  <img
                    class="cus-logo"
                    :src="AppFunction.getAppUrl(configData.invoice_logo)"
                    alt="NF"
                  />
                </div>
              </div>
              <div class="cus-w-50 cus-f-right cus-text-right">
                ...
                <table class="cus-f-right cus-w-75 cus-font-xm cus-mt-3">
                  ...
                  <tr>
                    ...
                    <td>:</td>
                    <td class="cus-text-right">
                      {{ formatDateToLocal(formData.date) }}
                    </td>
                  </tr>
                  <tr>
                    ...
                    <td>:</td>
                    <td class="cus-text-right">
                      {{ formatDateToLocal(formData.due_date) }}
                    </td>
                  </tr>
                </table>
            ...
            <div class="cus-invoice_container__item cus-p-5">
              <div class="cus-w-50 cus-f-left cus-font-xm">
                <p v-if="formData.created_by" class="cus-bold">
                  {{ $t("from") }}
                </p>
                <p>{{ formData.created_by.full_name }}</p>
                <template v-if="formData.created_by.profile">
                  <small v-if="formData.created_by.profile.contact"
                    >{{ $t("contact") }}:
                    {{ formData.created_by.profile.contact }}</small
                  >
                  <br />
                  <small v-if="formData.created_by.profile.address"
                    >{{ $t("address") }}:
                    {{ formData.created_by.profile.address }}</small
                  >
                  <br />
                  <small v-if="formData.created_by.trn"
                    >TRN: {{ formData.created_by.trn }}</small
                  >
                </template>
              </div>
              <div class="cus-w-50 cus-f-right cus-font-xm">
                <div class="cus-w-75 cus-f-right">
                  ...
                  <template v-if="formData.client.profile">
                    <small v-if="formData.client.profile.contact"
                      >{{ $t("contact") }}:
                      {{ formData.client.profile.contact }}</small
                    >
                    <br />
                    <small v-if="formData.client.profile.address"
                      >{{ $t("address") }}:
                      {{ formData.client.profile.address }}</small
                    >
                    <br />
                    <small v-if="formData.client.trn"
                      >TRN:
                      {{ formData.client.trn }}</small
                    >
                  </template>
                </div>
                <div class="cus-f-clear"></div>
              ...
              >
                <thead>
                  <tr class="cus-bg-dark cus-text-light">
                    <th class="cus-w-45 cus-p-1 cus-text-left">
                      {{ $t("product") }}
                    </th>
                ...
                <tbody>
                  <tr
                    v-for="(invoiceProduct, index) in formData.invoice_details"
                    ...
                    <td class="cus-p-1 cus-text-right">
                      {{
                        numberWithCurrencySymbol(
                          calculateProductTax(
                            invoiceProduct.tax ? invoiceProduct.tax.value : 0,
                  ...
                  <tr class="cus-bg-transparent">
                    ...
                    <td class="cus-text-right cus-p-1">
                      {{ numberWithCurrencySymbol(totalTax) }}
                    </td>
                  </tr>
                  <tr class="cus-bg-transparent">
                    <td colspan="2"></td>
                    ...
                    <td
                      ...
                      class="cus-text-right cus-p-1"
                    >
                      {{ formData.discount }} %
                    </td>
                    <td v-else class="cus-text-right cus-p-1">
                      {{ numberWithCurrencySymbol(formData.discount) }}
                    </td>
                  </tr>
                  <tr class="cus-bg-transparent">
                    <td colspan="2"></td>
                    <td colspan="2" class="cus-bold p-1">
                      {{ $t("total") }} :
                    </td>
                    <td class="cus-text-right cus-p-1">
