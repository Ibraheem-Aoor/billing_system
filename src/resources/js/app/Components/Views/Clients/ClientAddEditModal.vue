...
export default {
  ...
  methods: {
    ...
    afterSuccessFromGetEditData(response) {
      ...
      this.formData.phone = this.formData.user?.profile?.contact
        ? this.formData.user.profile.contact
        : "";
      this.formData.address = this.formData.user?.profile?.address
        ? this.formData.user.profile.address
        : "";
  ...
  computed: {
    names() {
      const full_name_spited = this.formData.full_name
        .split(" ")
        .filter((name) => name);
      if (full_name_spited.length) {
        if (full_name_spited.length === 2) {
          ...
          return {
            first_name: full_name_spited[0],
            last_name: full_name_spited
              .slice(1, full_name_spited.length)
              .join(" "),
          };
        }
      }
